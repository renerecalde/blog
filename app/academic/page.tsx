...
export function TitlesList() {
    return (
        <ul role="list" className="divide-y divide-gray-100">
            {titles.map((title) => (
                <li key={title.company} className="flex justify-between gap-x-6 py-5">
                    <div className="flex min-w-0 gap-x-4">
                        <div className="h-12 w-12 flex-none runded-full bg-gray-10">
                            {title.icon}
                        </div>
                        <div className="min-w-0 flex-auto">
                            <p className="text-sm font-semibold leading-6 text-white-900">{title.cargo}</p>
                            <p className="mt-1 text-xs leading-5 text-white-500"> {title.description} </p>
                        </div>
                    </div>
                    <div className="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
                        <p className="text-sm leading-6 text-white-900">{title.role}</p>
                        <p className="mt-1 truncate text-xs leading-5 text-white-500">{title.company}</p>
                        {title.lastSeen ? (
                            <p className="mt-1 text-xs leading-5 text-white-500">
                                Last seen <time dateTime={title.lastSeenDateTime}>{title.lastSeen}</time>
                            </p>
                        ) : (
